...
struct _FooDisplayPanelPrivate
{
...
static void rebuild_gui (FooDisplayPanel *self);
...
static gboolean output_info_supports_mode (FooDisplayPanel *self, GnomeRROutputInfo *info, int width, int height);
static char *make_resolution_string (int width, int height);
static GObject *foo_display_panel_constructor (GType                  gtype,
					      guint                  n_properties,
					      GObjectConstructParam *properties);

static void
foo_display_panel_get_property (GObject    *object,
                               guint       property_id,
                               GValue     *value,
                               GParamSpec *pspec)
{
...
static void
foo_display_panel_set_property (GObject      *object,
                               guint         property_id,
                               const GValue *value,
                               GParamSpec   *pspec)
{
...
static void
foo_display_panel_dispose (GObject *object)
{
  G_OBJECT_CLASS (foo_display_panel_parent_class)->dispose (object);
}

static void
foo_display_panel_finalize (GObject *object)
{
  FooDisplayPanel *self;

  self = FOO_DISPLAY_PANEL (object);
  ...
  G_OBJECT_CLASS (foo_display_panel_parent_class)->finalize (object);
}

static void
foo_display_panel_class_init (FooDisplayPanelClass *klass)
{
  GObjectClass *object_class = G_OBJECT_CLASS (klass);

  g_type_class_add_private (klass, sizeof (FooDisplayPanelPrivate));

  object_class->constructor = foo_display_panel_constructor;
  object_class->get_property = foo_display_panel_get_property;
  object_class->set_property = foo_display_panel_set_property;
  object_class->dispose = foo_display_panel_dispose;
  object_class->finalize = foo_display_panel_finalize;
}

static void
foo_display_panel_class_finalize (FooDisplayPanelClass *klass)
{
}

static void
error_message (FooDisplayPanel *self, const char *primary_text, const char *secondary_text)
{
  GtkWidget *toplevel;
  GtkWidget *dialog;

  if (self && self->priv->area)
...
static GnomeRRMode **
get_current_modes (FooDisplayPanel *self)
{
...
static void
rebuild_rotation_combo (FooDisplayPanel *self)
{
  typedef struct
  {
    GnomeRRRotation	rotation;
...
static int
count_active_outputs (FooDisplayPanel *self)
{
  int i, count = 0;
 ...
 */
static gboolean
mirror_screens_is_supported (FooDisplayPanel *self)
{
...
static void
rebuild_mirror_screens (FooDisplayPanel *self)
{
...
static void
rebuild_current_monitor_label (FooDisplayPanel *self)
{
  char *str, *tmp;
  GdkColor color;
  gboolean use_color;
...
static void
rebuild_on_off_radios (FooDisplayPanel *self)
{
  gboolean sensitive;
  gboolean on_active;
...
static void
rebuild_resolution_combo (FooDisplayPanel *self)
{
  int i;
  GnomeRRMode **modes;
  GnomeRRMode *mode;
...
static void
rebuild_gui (FooDisplayPanel *self)
{
...
static void
select_resolution_for_current_output (FooDisplayPanel *self)
{
  GnomeRRMode **modes;
  int width, height;
  int x,y;
...
static void
realign_outputs_after_resolution_change (FooDisplayPanel *self, GnomeRROutputInfo *output_that_changed, int old_width, int old_height)
{
...
static void
lay_out_outputs_horizontally (FooDisplayPanel *self)
{
  int i;
  int x;
  GnomeRROutputInfo **outputs;
...
static gboolean
output_info_supports_mode (FooDisplayPanel *self, GnomeRROutputInfo *info, int width, int height)
{
  GnomeRROutput *output;
  GnomeRRMode **modes;
  int i;
...
static GList *
list_connected_outputs (FooDisplayPanel *self, int *total_w, int *total_h)
{
  int i, dummy;
  GList *result = NULL;
  GnomeRROutputInfo **outputs;
...
static int
get_n_connected (FooDisplayPanel *self)
{
...
static double
compute_scale (FooDisplayPanel *self)
{
...
static void
set_top_bar_tooltip (FooDisplayPanel *self, gboolean is_dragging)
{
  const char *text;
...
static void
set_monitors_tooltip (FooDisplayPanel *self, gboolean is_dragging)
{
  const char *text;
...
static void
set_primary_output (FooDisplayPanel *self,
                    GnomeRROutputInfo *output)
{
  int i;
  GnomeRROutputInfo **outputs;
                 ...
                 gpointer data)
{
  ...
  FooDisplayPanel *self = g_object_get_data (G_OBJECT (area), "panel");
...
static PangoLayout *
get_display_name (FooDisplayPanel *self,
		  GnomeRROutputInfo *output)
{
  PangoLayout *layout;
  char *text;
...
static void
paint_output (FooDisplayPanel *self, cairo_t *cr, int i)
{
  int w, h;
  double scale = compute_scale (self);
  double x, y;
...
static void
foo_display_panel_init (FooDisplayPanel *self)
{
}

static GObject *
foo_display_panel_constructor (GType                  gtype,
                              guint                  n_properties,
                              GObjectConstructParam *properties)
{
  GtkBuilder *builder;
  ...
  obj = G_OBJECT_CLASS (foo_display_panel_parent_class)->constructor (gtype, n_properties, properties);
  self = FOO_DISPLAY_PANEL (obj);
  self->priv = FOO_DISPLAY_PANEL_PRIVATE (self);
...
void
foo_display_panel_edit_outputs (FooDisplayPanel *self,
                               gboolean        enable)
{
